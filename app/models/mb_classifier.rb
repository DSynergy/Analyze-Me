...
class MBClassifier
  ...
  def grabbing_data_from_apis(user)
    cleaned_tweets                 = clean_tweets(user.tweets.map(&:tweet))

    @introversion_classification   = classify_introversion(cleaned_tweets)
    @intuition_classification      = classify_intuition(cleaned_tweets)
    @thinking_classification       = classify_thinking(cleaned_tweets)
    @judging_classification        = classify_judging(cleaned_tweets)
    @sentiment_classification      = classify_sentiment(cleaned_tweets)
    @topic_classification          = classify_main_topics(cleaned_tweets)
    @age_classification            = classify_age(cleaned_tweets)
  end
  ...
  def parsing_data_from_apis(user)
    ...
    user.intuition                = grab_values_from_xml_document((((Nokogiri::XML(intuition_classification)),    3).to_f) + .25) * 100 # correction for API bias towards sensing
    user.sensing                  = grab_values_from_xml_document((((Nokogiri::XML(intuition_classification)),    1).to_f) - .25) * 100 # correction for API bias towards sensing
  ...
  def create_XML(text, username, classifier)
    ...
    <uclassify xmlns=\"http://api.uclassify.com/1/RequestSchema\" version=\"1.01\">
      ...
      <readCalls readApiKey=\"#{ENV['api_key']}\">
